...
// Defines which collision group an actor belongs to
enum class CollisionLayer { None, Border, Player, Ball, Goal };

// Base class for all game objects.
// Managed exclusively via shared_ptr — use Actor::create<T>() to instantiate.
class Actor : public std::enable_shared_from_this<Actor> {
   public:
    // Passkey idiom — prevents direct construction outside of create()
    struct Private {
        explicit Private() = default;
    };

    // Public but unforgeable constructor — only callable via create()
    Actor(Private) {
   ...
   protected:
    ...
    // Sets the drawable directly — safe to call from constructors
    void applyShape(std::variant<sf::RectangleShape, sf::CircleShape, sf::ConvexShape, sf::Sprite> drawable);

    // Sets velocity directly — safe to call from constructors
    void applyVelocity(const sf::Vector2f& vel);

    // Restores position to the previous frame's position
    void revertLocation();

    virtual void update(float deltaTime);

    // Returns shared_ptr to self — do NOT call from constructors
   ...
   public:
    ...
    // Factory method — creates a shared_ptr<T> using the passkey idiom
