...
std::shared_ptr<Actor> Actor::setShape(std::variant<sf::RectangleShape, sf::CircleShape, sf::ConvexShape, sf::Sprite> drawable) {
...
sf::Vector2f Actor::getCenter() const {
...
std::shared_ptr<Actor> Actor::setColor(const sf::Color& color){
    std::visit([&color](auto& shape) {
        if constexpr (!std::is_same_v<std::decay_t<decltype(shape)>, sf::Sprite>)
